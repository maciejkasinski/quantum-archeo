plot_calculate_selective_least_squares = function(df, sector0) {
  
  all_quanta <- seq(RNG_START, RNG_END, STEP)
  quanta_pairs <- combn(all_quanta, MAX_N_QUANTA)
  
  sizes <- df %>% filter(sector == sector0) %>% pull(size)
  ...
  for (i in 1:ncol(quanta_pairs)) {
    ...
    score <-
      calculate_selective_least_squares(sizes = sizes, quanta = c(quantum_1, quantum_2))
